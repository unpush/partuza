...
class PartuzaDbFetcher {
	...
	public function load_getActivities($ids, $appId, $sortBy, $filterBy, $startIndex, $count, $fields)
	{
		$this->checkDb();
		$activities = array();
	...
	public function deleteActivities($userId, $appId, $activityIds)
	{
		...
		foreach ($activityIds as $key => $val) {
			...
			$this->invalidate_dependency('activities', $val);
		}
	...
	public function setAppData($person_id, $key, $value, $app_id)
	{
		...
		$this->invalidate_dependency('appdata', $person_id);
		return true;
	}
	...
	public function load_getAppData($ids, $keys, $app_id)
	{
		...
		foreach ($ids as $key => $val) {
			if (! empty($val)) {
				$ids[$key] = mysqli_real_escape_string($this->db, $val);
				$this->add_dependency('appdata', $ids[$key]);
			}
		}
		if (! isset($keys[0])) {
			$keys[0] = '*';
	...
	public function load_getPeople($ids, $fields, $options)
	{
		foreach ($ids as $personId) {
			$this->add_dependency('people', $personId);
		}
	...
	/*
	 * Caching functions ... uses php's automagic call functions to overload function
	 * names, if a function is named load_<functionName> and the function name is
	 * listed in the $cachable array, the results of it are cached based on the
	 * md5 of the params parsed to it.
	 * Caching is DISABLED when debug = true, and ENABLED if debug = false
	 * 
	 * it uses the add_dependency / invalidate_depencency functions to track
	...
	public function __call($method, $arguments)
	{
		...
		if (in_array($method, $this->cachable) && !Config::get('debug')) {
